<template>
  <div class="blockchain-app">
    ...
    <Modal :showModal="showModal" :editedBlockDataForm="editedBlockDataForm" :newBlockDataForm="newBlockDataForm"
      :editedBlockIndex="editedBlockIndex" @closeModal="handleCloseModal" @addBlock="addBlock"
      @updateBlock="updateBlock" />

    <BlockchainTable :filteredChain="filteredChain" @editBlock="handleEditBlock" />
...
const filteredChain = computed(() => {
  ...
  return chain.value.filter((block) => block.data.product.toLowerCase().includes(query) && block.index !== 0);
});
